...
from medmnist import DermaMNIST,PathMNIST,OrganCMNIST,TissueMNIST
...
class Data:
    ...
    def get_alldata(self):
        ...
        return self.handler(self.X_train[labeled_idxs], self.Y_train[labeled_idxs], self.args_task['transform_train'])
...
def get_DermaMNIST(handler, args_task):
    ...
    for i in range(len(raw_train)):
        X_tr.append(np.array(raw_train[i][0]))
        Y_tr.append(np.array(raw_train[i][1])[0])
    for i in range(len(raw_test)):
        X_te.append(np.array(raw_test[i][0]))
        Y_te.append(np.array(raw_test[i][1])[0])
...
def get_PathMNIST(handler, args_task):
    raw_train = PathMNIST(split="train", download=True, size=28)
    raw_test = PathMNIST(split="test", download=True, size=28)
    X_tr=[]
    Y_tr=[]
    X_te=[]
    Y_te=[]
...
def get_OrganCMNIST(handler, args_task):
    raw_train = OrganCMNIST(split="train", download=True, size=28)
    raw_test = OrganCMNIST(split="test", download=True, size=28)
    X_tr=[]
    Y_tr=[]
    X_te=[]
    Y_te=[]
...
def get_ChestMNIST(handler, args_task):
    raw_train = TissueMNIST(split="train", download=True, size=28,)
    raw_test = TissueMNIST(split="test", download=True, size=28)
    X_tr=[]
    Y_tr=[]
    X_te=[]
    Y_te=[]
...
def get_SVHN_binary(handler, args_task):
    ...
    binary_train_targets = (data_train.labels != 0)  # 0 if class=0, 1 otherwise
    binary_test_targets = (data_test.labels != 0)    # 0 if class=0, 1 otherwise
    return Data(data_train.data, torch.from_numpy(binary_train_targets).long(), data_test.data, torch.from_numpy(binary_test_targets).long(), handler, args_task)
...
def get_CIFAR10_binary(handler, args_task):
    data_train = datasets.CIFAR10('./data/CIFAR10', train=True, download=True)
    data_test = datasets.CIFAR10('./data/CIFAR10', train=False, download=True)
    
    # Convert to binary classification: class 0 stays 0, all others become 1
    train_targets = torch.LongTensor(data_train.targets)
    test_targets = torch.LongTensor(data_test.targets)
    binary_train_targets = (train_targets != 0).long()  # 0 if class=0, 1 otherwise
    binary_test_targets = (test_targets != 0).long()    # 0 if class=0, 1 otherwise
    
    return Data(data_train.data, binary_train_targets, data_test.data, binary_test_targets, handler, args_task)

def get_CIFAR100_binary(handler, args_task):
    ...
    train_targets = torch.LongTensor(data_train.targets)
    test_targets = torch.LongTensor(data_test.targets)
    train_targets[train_targets >= 9] = 9
    test_targets[test_targets >= 9] = 9
    
    return Data(data_train.data, train_targets, data_test.data, test_targets, handler, args_task)

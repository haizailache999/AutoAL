...
class MedMNIST_Net_LPL(nn.Module):
	def __init__(self, dim = 28 * 28, pretrained=False, num_classes = 11):
		super().__init__()
		resnet18 = models.resnet18(pretrained=pretrained)
		self.features = nn.Sequential(*list(resnet18.children())[:-1])
		self.feature0 = nn.Sequential(*list(resnet18.children())[0:3])
		...
		self.feature0[0] = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1, bias=False)        #need config OrganC:1, Path:3
		self.dim = resnet18.fc.in_features
...
def get_lossnet(name):
	if name == 'PneumoniaMNIST':
		...
		#print("get get get",name)
		return LossNet(feature_sizes=[16,8, 4,2], num_channels=[64, 128, 256, 512], interm_dim=128)
	elif 'OrganCMNIST' or 'PathMNIST' or 'ChestMNIST' in name:
		return LossNet(feature_sizes=[28, 14, 7, 4], num_channels=[64, 128, 256, 512], interm_dim=128) 
	elif 'MNIST' in name:

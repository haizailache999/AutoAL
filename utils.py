...
from handlers import MNIST_Handler, SVHN_Handler, CIFAR10_Handler, openml_Handler, MNIST_Handler_joint, SVHN_Handler_joint, CIFAR10_Handler_joint,Medmnist_Handler,Medmnist_Handler_joint
from data import get_MNIST, get_FashionMNIST, get_EMNIST, get_SVHN, get_CIFAR10, get_CIFAR10_imb, get_CIFAR100,  \
								...
								get_DermaMNIST, get_PathMNIST, get_OrganCMNIST, get_ChestMNIST, get_CIFAR10_binary, get_SVHN_binary, get_CIFAR100_binary
from nets import Net, MNIST_Net, CIFAR10_Net, openml_Net, get_net_vae, MedMNIST_Net
from nets_lossprediction import Net_LPL, MNIST_Net_LPL, CIFAR10_Net_LPL, get_lossnet, MedMNIST_Net_LPL
...
from query_strategies import RandomSampling, LeastConfidence, MarginSampling, EntropySampling, \
								...
								LossPredictionLoss, VAAL, WAAL,ActiveLearningByLearning
...
def get_handler(name):
	if name == 'MNIST':
		...
		return MNIST_Handler
	elif name=='DermaMNIST' or name=='PathMNIST' or name=='OrganCMNIST' or name=='ChestMNIST':
		return Medmnist_Handler
		...
		return CIFAR10_Handler
	elif name == 'CIFAR100_binary':
		return CIFAR10_Handler
	elif name == 'CIFAR10_binary':
		return CIFAR10_Handler
...
def get_dataset(name, args_task):
	if name == 'MNIST':
		...
		return get_DermaMNIST(get_handler(name), args_task)
	elif name=='PathMNIST':
		return get_PathMNIST(get_handler(name), args_task)
	elif name=="OrganCMNIST":
		return get_OrganCMNIST(get_handler(name), args_task)
	elif name=="ChestMNIST":
		return get_ChestMNIST(get_handler(name), args_task)
	elif name == 'FashionMNIST':
		...
		return get_SVHN_binary(get_handler(name), args_task)
	elif name == 'CIFAR10':
		...
		return get_CIFAR10_binary(get_handler(name), args_task)
	elif name == 'CIFAR100_binary':
		return get_CIFAR100_binary(get_handler(name), args_task)
	elif name == 'TinyImageNet':
...
def get_net(name, args_task, device):
	if name == 'MNIST':
		...
		return Net(MedMNIST_Net, args_task, device)
	elif name=='PathMNIST' or name=='OrganCMNIST' or name=='ChestMNIST':
		return Net(MedMNIST_Net, args_task, device)
	elif name == 'FashionMNIST':
		return Net(MNIST_Net, args_task, device)
	elif name == 'EMNIST':
...
def get_net_lpl(name, args_task, device):
	...
	if name == 'MNIST':
		...
		return Net_LPL(MedMNIST_Net_LPL, args_task, device, loss_net)
	elif name == 'FashionMNIST':
		return Net_LPL(MNIST_Net_LPL, args_task, device, loss_net)
	elif name == 'EMNIST':
		return Net_LPL(MNIST_Net_LPL, args_task, device, loss_net)
...
def get_strategy(STRATEGY_NAME, dataset, net, args_input, args_task):
	...
	elif STRATEGY_NAME == 'ActiveLearningByLearning':
		return ActiveLearningByLearning(dataset, net, args_input, args_task)
	else:
		raise NotImplementedError
